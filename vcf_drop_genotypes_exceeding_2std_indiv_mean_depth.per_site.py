...
# usage example
# python vcf_drop_genotypes_exceeding_2std_indiv_mean_depth.per_site.py --vcf raw.01.vcf.tmp6.recode.vcf --gdepth out.gdepth

"""
1) takes vcf made by freebayes + out.gdepth made by "vcftools --vcf fufu.vcf --geno-depth"
2) for each individual, calculates mean + 2*stdev over all sites at depth >0 (i.e. no coverage = NOT counted); gets set of SITES that exceed this threshold -> removal/blacklisted genotypes
3) parses the vcf and sets blacklisted genotypes to ".:."

=> thresholds & filtering WORKS PER SITE, NOT PER CONTIG!!
	...
	with open(gdepth, "r") as INFILE:
		...
		for line in INFILE:
			...
			for sample, idx in samples_idx.items():
				depth = fields[idx]
				if not depth == "-1":
					samples_depths[sample].append(int(depth))
#				else:
#					samples_depths[sample].append( 0 )
...
#		print samples_depths
	...
	with open( gdepth + ".indiv_coverage_report.txt", "w") as OUTFILE:
		OUTFILE.write("\t".join( ["sample", "n_sites_covered", "depth_mean", "var", "sd", "removal_threshold"] ) + "\n")
		OUTFILE.write("\n".join(outlines))
