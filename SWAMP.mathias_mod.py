...
Mathias Scharmann modified to let it read and write another (relaxed) phylip format
...
def print_alignment_summary(infile):
    ...
    seq_dict = read_phylip_mathias(infile)
...
def sliding_window_scan_file(infile, threshold, windowsize, interscan,
    ...
    seqdict = read_phylip_mathias(infile)  # Create sequences dictionary
...
def read_phylip_original(infile):
...
def read_phylip_mathias(infile):
    '''Read in phylip sequences'''
    try:
        with open(infile) as readfile:
            sequences = {}
...
def print_masked_phyfile(phyinfile, masked_dict):
    ...
    write_phylip(phyoutfile, masked_dict)
...
def write_phylip (phyoutfile, seqdict):
	
	outlines = []
	
	ntaxa = len(seqdict.keys())
	len_align = len(seqdict[seqdict.keys()[0]])
	header = str(ntaxa) + " " + str(len_align)
	outlines.append(header)
	
	for id, seq in seqdict.items():
		outlines.append( id + "    " + seq )
	
	with open(phyoutfile, "w") as OUTFILE:
		OUTFILE.write("\n".join(outlines)+ "\n")
