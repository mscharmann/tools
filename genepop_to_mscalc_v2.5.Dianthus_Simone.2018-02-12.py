...
"""
for Dianthus,
both per-locus population mutation rate mu AND per-locus population intralocus recombination rate rho are set to:
loc_len * (2.5*10**(-8)) * 4 * 100000 
"""
...
def make_ms(ms_outfile, out_msdict, loci_ordered):
	...
	for locus in loci_ordered:  # keep loci in order
		...
		try: # some loci may not contain any segregating sites either because they were not polymorphic or because the sites were lost in filtering. Need to except that case!
			...
			positions_in_numerical_order = sorted( [int(x) for x in out_msdict[locus].values()[0].values()[0].keys()], key = int)
			lines.append("positions: {0}".format(" ".join([str(x) for x in positions_in_numerical_order ]))) # this is probably meaningless
		except IndexError:
...
#					for snp in out_msdict[locus][pop][sample].keys():
					for snp in [str(x) for x in positions_in_numerical_order]:
#						print snp
...
#	for loc_len in [ int(x.split("L")[1]) for x in loci_ordered]:  # keep loci in order  # keep loci in order
	for loc_len in [ int(locilength) for x in loci_ordered]:  # keep loci in order  # keep loci in order
		theta_values.append( str( loc_len* (2.5*10**(-8)) * 4 * 100000 ) )		
	line5 =	"\t".join(theta_values) + "\n"
	
	# intra-locus recombination rate rho is set to loc_len*2.5*10**(-8)) * 4 * 100000
	line6 = "\t".join([ str( loc_len* (2.5*10**(-8)) * 4 * 100000 ) ]*int(nloci)) 
