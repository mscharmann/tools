...
# Python 2.7.6
# vcf_drop_genotypes_exceeding_2std_indiv_mean_depth.v2.py
# 14 April 2015
# Mathias Scharmann
...
v2 added feature: report also surviving number of RADtags (those reamining AFTER dropping excessive coverage) 
...
#	un	2312	25	G	A	.	PASS	NS=17;AF=0.529,0.471	GT:DP:GL	1/1:32:.,.,.	
	...
	surviving_counter = {sample:[] for sample in samples}
						...
						try: # catch and except case where the record is only "." (e.g. as output by freebayes to indicate zero coverage)
							if not record.split(":")[1] == ".":
								...
								if int(dp) <= thresholds[ samples_vcf_idx[vcf_idx] ]:
									...
									surviving_counter[ samples_vcf_idx[vcf_idx] ].append( fields[0] )
								else:
...
def write_report (samples_depths, surviving_counter, gdepth):
	...
	for sample in surviving_counter.keys():	
		...
		outlines.append("\t".join( [ str(x) for x in [ sample, len(depths), mean, var, sd, thresholds[sample], len(set(surviving_counter[sample])) ] ] ) )

	
	with open( gdepth + ".indiv_coverage_report.txt", "w") as OUTFILE:
		OUTFILE.write("\t".join( ["sample", "n_RADtags_covered_before", "depth_mean", "var", "sd", "removal_threshold", "n_RADtags_surviving"] ) + "\n")
		OUTFILE.write("\n".join(outlines))
...
surviving_counter = parse_vcf(args.vcf, thresholds)
